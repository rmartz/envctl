...
const config = JSON.parse(
  await readFile(join(repoRoot, ".releaserc.json"), "utf8"),
);
...
const context = {
  cwd: repoRoot,
  env: {},
  options: { repositoryUrl: "https://github.com/rmartz/envctl.git" },
...
try {
  ...
  if (!notes || notes.trim().length === 0) {
    console.error(
      "Changelog render produced empty notes — the toolchain is misconfigured.",
    );
    process.exit(1);
  }
