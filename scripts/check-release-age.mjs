#!/usr/bin/env node
/**
 * Dependency release-age gate. Fails a PR that introduces a package version
 * younger than RELEASE_AGE_MIN_DAYS (default 7). This is a second, deterministic
 * layer on top of Dependabot's `cooldown` (`.github/dependabot.yml`), which is
 * advisory at PR-creation time and has documented reliability gaps for the npm
 * ecosystem that bypass the window (dependabot/dependabot-core#12677) — e.g.
 * multiple same-day patch bumps merging while each is only days old, well inside
 * the configured cooldown. This gate enforces the "let a release age before we
 * adopt it" policy at the layer we control, catching those escapes.
 *
 * Why this shape and not pnpm's `minimum-release-age` in committed config:
 * putting that setting where Dependabot's own lockfile regeneration reads it
 * forces pnpm to fetch publish-time metadata for the whole candidate tree on
 * every update — a full-metadata storm that causes multi-minute Dependabot
 * timeouts. This gate never touches Dependabot's resolver: it inspects the
 * *result* (the lockfile diff) and queries the registry for only the handful of
...
if (!isFinite(minDays) || minDays <= 0) {
  console.error(
    `RELEASE_AGE_MIN_DAYS must be a positive finite number (got: ${process.env.RELEASE_AGE_MIN_DAYS ?? ""})`,
  );
  process.exit(1);
}
