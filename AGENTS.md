...
truth. `CLAUDE.md` is a thin wrapper that imports it, so there is exactly one
place to edit and no duplicate copy to fall out of sync.

- **All directives go in `AGENTS.md`.** Write and edit conventions here, never
  in `CLAUDE.md`.
- **Pairing.** Every `AGENTS.md` must have a companion `CLAUDE.md` in the same
  directory, and every `CLAUDE.md` must have a companion `AGENTS.md`.
- **`CLAUDE.md` is a bare wrapper.** Its only content is the Claude Code import
  line `@AGENTS.md` (blank lines aside) — no directives, no other text. Use the
  `@` import, **not** a symlink: a symlinked `CLAUDE.md` is rejected.

  Maintaining `CLAUDE.md` as a full copy of `AGENTS.md` invites silent drift —
  the two diverge and it stops being clear which one an agent actually reads.
